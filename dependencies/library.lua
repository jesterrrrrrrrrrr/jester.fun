...
function Components.Paragraph(parent, title, content, theme, layoutOrder)
	local textHeight = Util.GetTextSize(content, 13, Font.fromEnum(Enum.Font.SourceSans), Vector2.new(300, math.huge))
	...
	function paragraphObj:SetText(newContent)
		if newContent then
			...
			local newHeight =
				Util.GetTextSize(newContent, 13, Font.fromEnum(Enum.Font.SourceSans), Vector2.new(300, math.huge))
...
function Xan:CreateWindow(config)
	...
	local function createSearchResultItem(elementData, index)
		...
		local typeBadgeText = Util.Create("TextLabel", {
			...
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 1, 0),
			Font = isButtonType and Enum.Font.SourceSans or Enum.Font.SourceSansBold,
